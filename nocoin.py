...
def zip_file(zip_path, zip_name, main_directory, definitions, _totals_):
    tmp_dir = main_directory + '/temp_unzips'
    z_level = [[zip_name, zip_path, os.path.join(tmp_dir, os.path.splitext(zip_name)[0])]]
    try:
        for z_list in z_level:
            if not os.path.exists(z_list[2]):
                os.makedirs(z_list[2])
            z_file = tarfile.open(z_list[1]) if tarfile.is_tarfile(z_list[1]) else ZipFile(z_list[1])
            with z_file as zip_dir_path:
                zip_dir_path.extractall(path=z_list[2])
                for (tmp_dir_name, tmp_sub_dir, tmp_file_name) in os.walk(z_list[2], topdown=True):
                    for zip_file_name in tmp_file_name:
                        ...
                        if zipfile.is_zipfile(zip_file_name) or tarfile.is_tarfile(tmp_dir_path):
                            z_level.append([zip_file_name, tmp_dir_path, os.path.splitext(tmp_dir_path)[0]])
                        else:
                            file_check(tmp_dir_path, zip_file_name, main_directory, definitions, _totals_, zip_name)
        shutil.rmtree(tmp_dir)
    except Exception as e:
        log(zip_path + '  Is not a valid zipfile', str(e))
...
def get_definitions():
    ...
    for numb in range(0, 6):
        ...
        if user_def:
            path_exclude.append(user_def)
            default_defs.append(user_def)
    for def_list in default_defs:
        log('    ' + def_list)
        try:
            definitions = open_url(def_list).splitlines() if 'http' in def_list else open(def_list, 'rb')
                ...
                if line and not line.startswith('#'):
                    for pattern in ('[^/]*//([^$]*)', '\s([^$]*)', '(\d+\.+)\s', '([^$]*)'):
                        ...
                        if match:
                            ...
                            if match not in defs and '0.0.0.0' not in match:
                                defs += str(match)
                                if 'http' in def_list:
                                    defs += '\n'
        except Exception as e:
...
def nocoin():
    ...
    if os.path.exists(log_file):
        os.remove(log_file)
    log('Please Note: False Positives in Addon Cache .db Files Are Possible Due To The Way Addons Cache Web Pages\n'
        'Please Check With The Addons Developer For Adding Filters To Remove These Instances If They Occur\n')
    ...
    for directory_to_test in directories:
        for (main_directory, main_sub_dir, main_files) in os.walk(directory_to_test, topdown=True):
            for main_file_name in main_files:
                ...
                if main_file_path in path_exclude:
                    continue
                # if called from service or auto run, skip dp reporting
...
def nc_options(nc_path):
    ...
    with open(nc_path, 'rb') as temp_file:
        ...
        if not match:
            ...
            return
        for occurance, addon_name, file_name, path_name in match:
            status = ''
            if addon_name != 'Unknown':
                status = ' Enabled:  ' if xbmc.getCondVisibility("System.HasAddon(%s)" % addon_name) else ' Disabled;  '
            if not report_list and os.path.isfile(path_name):
                report_list = [[occurance, addon_name, file_name, path_name, status]]
            else:
                if path_name not in str(report_list) and os.path.isfile(path_name):
                    report_list.append([occurance, addon_name, file_name, path_name, status])
        index = dialog.select('Choose an Instance', ['%s:%s  %s' % (inst[1], inst[4], inst[2]) for inst in report_list])
        if index > -1:
            i_addon = str(report_list[index][1])
            i_file = str(report_list[index][2])
            i_path = str(report_list[index][3])
            if i_addon == 'Unknown':
